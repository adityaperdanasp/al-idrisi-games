...
function startRoamAnimation() {
  ...
  function frame(now) {
    ...
    if (cdist < 6) {
      ...
      // so the nose always points the way it's actually moving.
...
// scheduled independently of the rAF loop on a random interval.
let roamSmokeTimeoutId = null;
function scheduleRoamSmoke() {
  roamSmokeTimeoutId = setTimeout(() => {
    if (roamState) spawnRoamSmoke();
    scheduleRoamSmoke();
  }, 1800 + Math.random() * 2400);
}
function spawnRoamSmoke() {
  const puff = document.createElement("div");
  puff.className = "roam-smoke";
  puff.textContent = "💨";
  puff.style.left = roamState.car.x + "px";
...
// Generic (no name baked in) praise/encourage MP3s, played locally (no API
// calls). Clips 1..NAME_FIRST_COUNT are phrased for "name, then line" (e.g.
// "Arsya! That's exactly right!"); clips after that are phrased for
// "line, then name" (e.g. "That's exactly right, Arsya!") — mixing both
// orders so the cheer doesn't always sound the same shape. Falls back to
// browser TTS if a file fails to load/play (e.g. offline, blocked audio, or
// no recorded clip yet for this player's name).
...
function playCheerAudio(kind, fallbackText) {
  ...
  if (kind === "praise") {
    let poolNum;
    do {
      poolNum = rand(1, PRAISE_POOL_SIZE);
    } while (poolNum === lastCheerAudioNum.praise);
    lastCheerAudioNum.praise = poolNum;
    ...
    playSplicedCheer(kind, fallbackText, poolNum - PERSONAL_PRAISE_COUNT);
    return;
  }

  playSplicedCheer(kind, fallbackText);
}

function playSplicedCheer(kind, fallbackText, forcedNum) {
  let num = forcedNum;
  if (!num) {
    do {
  ...
  const nameFirst = num <= NAME_FIRST_COUNT[kind];
  ...
  if (nameFirst) {
    ...
    currentCheerAudio = line;
    line.addEventListener("ended", () => { currentCheerAudio = nameClip; nameClip.play().catch(() => {}); });
    line.addEventListener("error", () => speak(fallbackText));
    line.play().catch(() => speak(fallbackText));
  }
}
