...
9. **Mastery-streak fix buat weak-topic weighting (poin 7)**: `weightedRand()` di `multipleazka/script.js` sekarang punya `MASTERY_STREAK = 5` — kalau `topicStats.streak` (dicatat `leaderboard.js`'s `recordTopicAttempt`, reset ke 0 tiap kali salah) udah ≥5, bobot topik itu balik ke baseline berapa pun `wrong` historisnya. `updateLocalTopicStat()` bikin ini efektif LANGSUNG dalam 1 sesi race yang sama (gak perlu reload). Fix persis sesuai permintaan Adit (contoh: "5x bener berturut-turut = udah bisa").
10. **AI Tutor — dari mockup jadi BENERAN JALAN**: dibikin `api/generate-hint.js` (Vercel serverless, Claude Haiku 4.5, pola sama kayak `api/generate-insight.js` — `ANTHROPIC_API_KEY` server-side only, gagal = balik `{error}` gak pernah nge-block game). Ketiga game di-wire: tiap kali jawaban salah, detail soal (`question, correctAnswer, kidAnswer, topic`) disimpen di `state.lastWrong` (multipleazka) / `session.lastWrong` (azkacraft) / `state.lastWrong` (azkauniverse). Pas layar hasil/reward kebuka, `fetch("/api/generate-hint")` beneran manggil AI. Kalau gak ada soal yang salah sepanjang sesi, atau API call gagal (network/key issue) → card `#ai-hint-card` disembunyiin, gak pernah nge-block UI. Sudah ditest: fail gracefully dikonfirmasi via local static server (gak ada endpoint beneran → card hilang bersih, gak error visual). **Sudah commit (`a6a398d`) + deploy ke hub & ketiga standalone domain.** Adit udah setup `ANTHROPIC_API_KEY` di Vercel (Production) — tinggal ditest langsung di `playalidrisi.fun` pake jawaban salah beneran buat liat hint AI-nya nongol.
11. **(Sesi lain, bukan saya)**: dashboard di-rebuild total per "Sirka design handoff" — charts, KPI, AI-generated insight pakai `api/generate-insight.js`. Belum saya review detail.

## Yang masih perlu ditindaklanjuti
1. **Verifikasi live di `playalidrisi.fun`**: coba jawab salah beneran di salah satu game, pastikan hint AI beneran nongol (bukan cuma gagal-gracefully-hidden) — belum divalidasi end-to-end dengan API key asli, cuma ditest local (yang emang selalu gagal karena gak ada serverless function jalan di static server).
2. Testing multiplayer 2-device fisik beneran (disconnect handling, BGM stop/resume) — masih cuma divalidasi via kode+simulasi.
3. Cek arah icon vehicle lain (ship/train/truck/bus) — cuma plane yang udah dicek & difix (kebalik arahnya).
4. Dashboard: `parentEmail` baru keisi 1 dari ~25 murid (perlu Adit isi manual biar insight email bisa jalan ke semua ortu). Konfirmasi dari Adit: email approval **sudah pernah nyampe** ke inbox asli (gak perlu diverifikasi ulang).
5. Review hasil rebuild dashboard dari sesi lain (poin 11 kerjaan selesai) — belum sempat dicek detail dari sisi sesi ini.
6. Kalau AI Tutor hint mau dipakai jangka panjang, pantau cost Anthropic API (estimasi awal <$5/bulan buat kelas ini) — belum ada monitoring/alert biaya.
