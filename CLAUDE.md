...
**Entry point**: node "Glass Bridge Challenge" (icon 🧊, tag "Mini-Game") di paling bawah Storybook Trail (`#screen-map`'s `#bookshelf`, di-append manual di `renderBookshelf()` setelah loop chapter asli) — BUKAN bagian dari `QUESTION_BANK.chapters` (gak ada stars/lock system, selalu unlocked). Sempet sempet ditaro sebagai card ke-3 di landing page mode-grid (sejajar Solo Adventure/Multiplayer), tapi dicabut dari situ begitu dipindah ke trail biar cuma ada 1 entry point. Tombol "Exit" di dalemnya balik ke `screen-map` (bukan `screen-landing`), nyesuain jalur masuknya.
