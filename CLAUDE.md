...
- **Entry point**: icon 🥷 di topbar MathVille (`btn-ninja`, visibility sama kayak `btn-map`/`btn-drive` — hidden di landing/pair, muncul di semua screen lain) → `launchNinjaRunner()` → `#screen-ninja`. **Juga ada card sendiri di landing page hub** (di bawah Focus Round, warna gold, subtitle "Pick your favourite subject") → deep-link `mathville/?ninja=1` (pola sama kayak roaming car's `?drive=1` dan Focus Round's `?focus=1`). ⚠️ **Gotcha yang ketemu & difix**: deep-link check ini HARUS ditaro SETELAH section Ninja Runner (bukan digabung sama `?drive=1`/`?focus=1` checks yang lebih awal di file) — `ninjaState` di-declare pake `let` jauh lebih akhir, manggil `launchNinjaRunner()` sebelum baris deklarasi itu ke-eksekusi throw `ReferenceError: Cannot access 'ninjaState' before initialization` (temporal dead zone).
- **Run-lane encounter (2026-08-05, direvisi jadi mekanik beneran)**: awalnya PURELY DEKORATIF (timer 10 detik tetap, jump gak ngefek apa-apa, per permintaan eksplisit sebelumnya) — **udah dibalik lagi per feedback baru**: sekarang 2 tahap beneran. (1) Batu mendekat (`NINJA_OBSTACLE_MS=1.8s`) — tap JUMP kapan aja selama pendekatannya bikin `ninjaState.obstacleDodged=true`, di-resolve di `ninjaResolveObstacle()` jadi class `.dodged` (poof+fade) atau `.bumped` (goyang+fade) tergantung sempet jump apa nggak. (2) Musuh mendekat (`NINJA_ENEMY_MS=1.8s`) — animasinya BENERAN SELESAI (nyampe ke posisi ninja) itu yang jadi trigger munculnya kartu subject (`ninjaResolveEnemy()` → `renderNinjaGates()`), gantiin `setTimeout` buta yang lama. Tetep gak ada fail-state/life-system (dodge atau kena batu sama-sama lanjut ke musuh) — cuma sekarang jump beneran ada efek keliatan, bukan kosmetik doang.
